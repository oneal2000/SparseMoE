...
def main():
    ...
    elif args.method == "token_old":
        model = TokenRAG_old(args)
    else:
        raise NotImplementedError

    logger.info("start inference")
    for i in tqdm(range(len(data))):
        last_counter = copy(model.counter) # model is TokenRAG here
        batch = data[i]
        pred, rel_psgs = model.inference(batch["question"], batch["demo"], batch["case"])
        pred = pred.strip()
        ret = {
            "qid": batch["qid"], 
            "prediction": pred,
            "rel_psgs": list(rel_psgs)
        }
        if args.use_counter:
