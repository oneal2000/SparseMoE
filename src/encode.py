...
def get_train_data(aug_model, augments, tokenizer, args): # 生成模型训练所需的数据
    from prompt_template import get_prompt
    prompt_ids = []
    for aug in augments:
        psg = aug["passage"] # 原始的 passage
        rew = aug[f"{aug_model}_rewrite"] # 该 passage 的重写版本
        qas = aug[f"{aug_model}_qa"] # 对应的 QA 对，包含问题和答案
        qpa_cnt = (len(qas) + 1) // 2 # 计算问题的数量的一半（向下取整）。这一步的目的是将问题集分成两类：
        for qid, qa in enumerate(qas):
            if qid < qpa_cnt: # 第一类：处理前半部分问题，生成包含 passage 和 rewrite 的 prompt
                for ppp in [psg, rew]:
                    prompt_ids.append(get_prompt(tokenizer, qa["question"], 
                                                    [ppp], # 区别在于前半部分传入了psg和rewrite
                                                    qa["answer"] if not args.with_cot else qa["full_answer"], 
                                                    with_cot=args.with_cot))
            else: # 第二类：处理后半部分问题，仅使用问题和答案生成 prompt
                prompt_ids.append(get_prompt(tokenizer, qa["question"], 
                                                None, 
                                                qa["answer"] if not args.with_cot else qa["full_answer"], 
                                                with_cot=args.with_cot))
...
def main(args):
    ...
    if args.with_cot: # 如果 args.with_cot 为 True，则调用 prompt_template.get_fewshot 来设置少量样本（few-shot）提示（prompt）
    ...
    cot_name = "cot" if args.with_cot else "direct" # 根据 args.with_cot 来确定 CoT 模式的名称，如果启用了 CoT，则名称为 "cot"，否则为 "direct"
    for filename, fulldata in data_list: # full_data来自data_list
        ...
        for did, data in tqdm(enumerate(fulldata), total=len(fulldata)): # data来自full_data
            ...
            for pid in range(len(augment)):
                ...
                model = train(data["question"], [augment[pid]], args, model, tokenizer, 
                            init_adapter_path, save_path) # augment[pid]是召回的dpr里面的passage
